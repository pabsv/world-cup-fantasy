...
## Status
- **v2 built 2026-06-08.** Repo: https://github.com/pabsv/world-cup-fantasy (public). Working: auth (email+password), `/leaderboard` (titled "League"), `/picks/[userId]`, `/admin`.
- **Routing (restructured 2026-06-08):** `/` = **one shared landing** for logged-in AND logged-out (same marketing page; only the CTA target differs — `/login` vs `/predict`). `/predict` = a **hub** (hero + a single horizontal card "Full World Cup Prediction" + an "Upcoming competitions" section; **no Group Stage option** — it's derived from the full prediction). The card → **`/predict/full`** = the builder (groups → 8 thirds → full knockout bracket → champion, autosave).
- **Naming:** the `og_full` competition is shown to users as **"Full World Cup Prediction"** everywhere (DB `competitions.title` updated via migration `20260608120004_rename_full_prediction.sql`; UI strings too). The internal `type = 'og_full'` enum is unchanged. The **Group Stage tab is hidden** on `/leaderboard` (still scores behind the scenes). One known stale string left on purpose: the locked-banner inside `FullRunBuilder.tsx` still says "OG Full Run" — deferred to avoid colliding with a concurrent bracket-UI change.
